...
### Added
- **Admin global-config API** — `PUT /admin/config` updates `config.json` from an admin session (validated, merged into the existing config, hot-reloaded immediately). Manages `allowedOrigins`, `allowPublicSessions`, `safeMode`, `sessionTtlMs` and the per-IP `publicSessionLimiterConfiguration`. Surfaced as a **Settings** page in the console.
