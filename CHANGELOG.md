...
- PDF extraction (word-based path): a narrow leftmost label column that never shares a row with its neighbour — e.g. a `Classification` column (Actions / Mixtes / Autres) whose label sits on the value row while the security name wraps on its own rows — is no longer swallowed by the name column. The single-densest-row grid can't see such a column; it's now re-split apart, tightly guarded (leftmost column only; a moderate, data-evidenced, vertically-segregated gap) so every other report stays byte-identical (verified against the baseline snapshots).
